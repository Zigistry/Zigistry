...
export default function Z2j_app() {
  ...
  function hotReload() {
    ...
    try {
      JSON.parse(answer);
      const outputAnalysis = document.getElementById("outputAnalysis") as HTMLElement;
      outputAnalysis.innerHTML = "<span class='text-bold text-green-500'>Json seems good! &#x2713;</span>";
    } catch (e) {
      const outputAnalysis = document.getElementById("outputAnalysis") as HTMLElement;
  ...
  function download() {
    ...
    element.setAttribute('href', 'data:text/plain;charset=utf-8,' + encodeURIComponent(outputValue));
  ...
  return (
    <>
      <div className="mt-2 flex h-[60px] space-x-2 pl-2" >
        <button id="downloadJson" onClick={download} className="rounded bg-slate-400 px-7 py-4 text-black focus:border-4 focus:border-sky-400 dark:bg-blue-800 dark:text-white">
          Download .json
        </button>
      ...
      <div className="flex h-[calc(100vh-192px)] w-screen" >
        <div className="block w-1/2" >
          <div
            id="pseudo_editor"
            className="absolute left-0 top-[122px] z-10 m-2 h-[calc(100vh-200px)] w-1/2 overflow-y-scroll text-wrap break-words rounded-lg bg-[#2a3038] p-4 text-left font-mono text-white"
          ></div>
          <textarea
            ...
            autoCorrect="false"
            autoComplete="false"
            className="absolute  left-0 top-[122px]  z-20 m-2 h-[calc(100vh-200px)] w-1/2 border-r-2 border-white bg-transparent p-4 font-mono text-transparent caret-white"
          ></textarea>
        </div>
        <div className="ml-10 mr-2 mt-[9px] block h-[calc(100vh-200px)] w-1/2 overflow-y-scroll text-wrap break-words rounded-lg bg-[#263554] p-4 font-mono" >
          ...
          <div id="outputAnalysis" className="absolute w-[calc(50vw-50px)] rounded-lg bg-slate-800 p-5">
