...
export default async function recommendation_backend() {
  ...
  const gaming_response = await fetch("https://raw.githubusercontent.com/RohanVashisht1234/zigistry/main/database/out/games.json");
  if (!gaming_response.ok) throw new Error(`Error: ${gaming_response.statusText}`);
  const gaming_items: Repo[] = await gaming_response.json();

  const web_response = await fetch("https://raw.githubusercontent.com/RohanVashisht1234/zigistry/main/database/out/web.json");
  if (!web_response.ok) throw new Error(`Error: ${web_response.statusText}`);
  const web_items: Repo[] = await web_response.json();

  const gui_response = await fetch("https://raw.githubusercontent.com/RohanVashisht1234/zigistry/main/database/out/gui.json");
